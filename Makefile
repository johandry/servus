...
# Title:  Servus, a Serf extension
#
# Usage: make [<rule>]
#
# Basic rules:
# 		<none>		If no rule is specified will do the 'default' rule which is 'build'
#			build     Build a container to build and ship Servus.
...
# Servus binary for this OS and Architecture
$(BINARY): build

# build Servus. The binary is located in $(BIN)/servus
build: vendors test
...
# build Servus for a Docker image
build4docker: vendor-init test
	@[[ -d /go ]] || ( $(ECHO) "$(C_RED)$(I_CROSS) This rule is to be executed only inside the Servus build image$(C_STD)"; false )
...
# executes Servus in the servus container. This executes only one node, to have
# a cluster use `make cluster`
run:
	@$(ECHO) "$(C_GREEN)Running Servus from $(C_YELLOW)$(DOCKER_CON)$(C_STD)"
	docker run --name $(DOCKER_CON) --rm -it $(DOCKER_IMG)

# creates a Servus container and login into it. This may require to modify the
# Dockerfile to use a different base and maybe the rule to use a different shell
sh:
...
clean:
	...
	@$(ECHO) "  $(C_YELLOW)$(I_BULLET)$(C_GREEN) Servus image $(C_YELLOW)$(DOCKER_IMG)$(C_STD)"
